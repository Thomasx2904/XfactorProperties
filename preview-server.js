...
function startListingRefresh(response, scope = "national", mode = "manual") {
  ...
  const nationalPrompt = [
    ...
    "Do not add Tasmania, rentals, sold listings, off-market pages, generic suburb searches, duplicate records, duplicate addresses already in app.js, duplicate titles/suburbs on another portal, or pages without a usable direct listing URL and real listing hero image.",
    ...
    "If any existing listing is sold, under offer, under contract, leased, withdrawn, off market, or no longer active, set status so it disappears from the app.",
  ...
  const saPrompt = [
    ...
    "Use the same inclusive approach as the Moana apartment example: include strong beachfront apartments, units, townhouses, and villas even when no land/block size is published. Do not reject a listing solely because it is strata/apartment stock or because landSize is unknown; set landSize to null so price per sqm shows as TBA.",
    "Use portal suburb result pages first, especially realestate.com.au/buy/in-{suburb,+sa}/list-1 and matching Domain/Homely suburb sale pages, then open each promising result card and save only the exact listing URL. Check page 2 if present, nearby exact suburb name variants, and Esplanade/beachfront keyword searches before moving on. Also use direct listing pages on realestate.com.au, Domain, and Homely. Do not add rentals, sold listings, off-market pages, generic suburb searches, duplicate records, duplicate addresses already in app.js, duplicate titles/suburbs on another portal, or pages without a usable direct listing URL and real listing hero image.",
    ...
    "For each verified listing, also check available 5-year suburb/comparable sales growth evidence. If credible evidence supports potential above 15% annual growth, add investmentStar: true plus investmentAnnualGrowth5Yr or investmentFiveYearGrowth and a short investmentReason. If evidence is missing or weaker, leave investmentStar false or omit it; do not guess.",
    "If any existing SA listing is sold, under offer, under contract, leased, withdrawn, off market, or no longer active, set status so it disappears from the app.",
    `Update README counts if the total changes. Run ${nodePath} --check app.js before finishing. Keep the final response short.`
  ].join(" ");
...
function startLargeListingRefresh(response, mode = "manual") {
  ...
  const prompt = [
    ...
    "Target land size between 20 hectares and 2000 hectares inclusive. Do not require beachfront. Prefer properties with good capital-growth potential, access to major cities/regional centres, subdivision or tourism upside, water access, coastal proximity, income potential, or rezoning/development signals.",
    ...
    "Do not add rentals, sold listings, off-market pages, generic search pages, duplicate records, duplicate addresses already in app.js, duplicate titles/regions on another portal, or pages without a usable direct listing URL and real listing hero image.",
    ...
    "If any existing large-property listing is sold, under offer, under contract, leased, withdrawn, off market, or no longer active, set status so it disappears from the app.",
    `Update README counts if the total changes. Run ${nodePath} --check app.js before finishing. Keep the final response short.`
  ].join(" ");
